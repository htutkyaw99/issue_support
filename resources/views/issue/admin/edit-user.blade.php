...
@section('content')
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <div>
            <h1 class="text-2xl font-bold dark:text-white mb-3">Edit User</h1>
        </div>
        <form class="max-w-md" action="{{ route('admin.users.update', ['id' => $user->id]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="relative z-0 w-full mb-5 group">
                ...
                <select id="countries" name="role"
                    ...
                    @foreach ($roles as $role)
                        <option {{ $user->role === $role ? 'selected' : '' }} value="{{ $role->value }}">
                            {{ ucfirst($role->value) }}
                        </option>
                    @endforeach
                </select>
